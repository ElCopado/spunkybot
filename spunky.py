...
# Malondon's codeBlock START
                    ...
                    """cc = []
                    cc = [None]*(len(player_list1)+len(player_list2))
		    if len(player_list1)>=len(player_list2):
                        ...
                        cc[::2]=player_list2
                        cc[1::2]=player_list1
                    lenlist=math.floor(len(cc)/2)
                    for x in range(int(lenlist)):
                        if cc[x].get_team()==team1:
                            self.game.rcon_forceteam(cc[x].get_player_num(), Player.teams[team2])
                        else:
                            self.game.rcon_forceteam(cc[x].get_player_num(), Player.teams[team1])
                        pass"""
		    if team1 > team2:
...
#kick_original              if victim.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                            if victim.get_admin_role() > self.game.players[sar['player_num']].get_admin_role():
